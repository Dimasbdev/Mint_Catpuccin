...
SOCKET_PATH = f"/tmp/bento-control-{os.getuid()}.sock"
DATA_DIR = os.path.expanduser("~/.local/share/bento-control")

class BentoControlWindow(Gtk.Window):
    def __init__(self):
        super().__init__(type=Gtk.WindowType.TOPLEVEL)
        self.set_title("BentoControlCenter")
    ...
    def on_control_message(self, ucm, js_result):
        ...
        try:
            ...
            if action in ("ready", "refresh"):
                ...
                self.hide_popup()
                subprocess.Popen([os.path.expanduser('~/.local/bin/bento-clip')])
